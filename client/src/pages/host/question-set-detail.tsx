    ...
    reader.onload = (event) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (["multiple_choice", "true_false", "open"].includes(firstCol)) {
          ...
          [, text, optA, optB, optC, optD, correct, cat] = cols;
          const rawDiff = cols[8] as string;
          diff = (rawDiff === "easy" || rawDiff === "hard") ? rawDiff : "medium";
          pts = cols[9] || "100";
          timeLimit = cols[10] || "30";
        } else {
          qType = "multiple_choice";
          diff = "medium";
          [text, optA, optB, optC, optD, correct, cat] = cols;
          const rawDiff = cols[7] as string;
          diff = (rawDiff === "easy" || rawDiff === "hard") ? rawDiff : "medium";
          pts = cols[8] || "100";
          timeLimit = cols[9] || "30";
        }
        ...
        const points = pts ? parseInt(pts) || 100 : 100;
        const timeLimitSeconds = timeLimit ? parseInt(timeLimit) || 30 : 30;

        if (qType === "true_false") {
          questions.push({ questionType: qType, text, options: ["True", "False"], correctAnswer: correct, category: cat || undefined, difficulty: diff, points, timeLimitSeconds });
        } else if (qType === "open") {
          questions.push({ questionType: qType, text, options: [], correctAnswer: correct, category: cat || undefined, difficulty: diff, points, timeLimitSeconds });
        } else if (text && optA && optB && optC && optD && correct) {
          questions.push({ questionType: "multiple_choice", text, options: [optA, optB, optC, optD], correctAnswer: correct, category: cat || undefined, difficulty: diff, points, timeLimitSeconds });
        }
      }
